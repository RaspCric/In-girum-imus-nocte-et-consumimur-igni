...
4. [Mise en lien d’Active Directory et notre GLPI](#IV)
    ...
    5. [Gestion générale de GLPI : mise en pratique](IVE)
...
L'idée sera de créer un WServer qui servira de server entre notre GLPI et les usagers via un AD. Pour commencer il faudra donc monter un WServer propre et ensuite paramétrer l'AD pour gérer notre server GLPI.
...
- <span style="color: #32977E">=MINUSCULE(CONCATENER(STXT(A3;1;1);B3;"@tssr.lan"))</span> : création d'un mail <br/>
...
Ajouter un support glpi dans WServer2019 + une redirection DNS
...
#### E) Gestion générale de GLPI : mise en pratique --- <a name="IVE"></a>


* Téléchargement de FusionInventory Agent
* On l'installe sur notre Wserver 
* On renseigne le serveur GLPI : *http://192.168.0.32/plugins/fusioninventory/*
* On va sur http://localhost:62354 
* On force une remontée
* Ensuite on va sur le GLPI de son hôte et dans Ordinateur on peut voir la remontée **DC1**
